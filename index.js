...
function get20s(array){
  let newArray = [];
 for(let i = 0 ; i <= array.length ; i++) { if(`${array[i].years}` >= 1900 && `${array[i].years}` <= 2000 ) {
...
function addArtist(array,name,id,years,genre,nationality,bio){
  array.push({name,id,years,genre,nationality,bio})
  return array
}
...
function lotsOfArt(array){
  ...
  for(let i = 0 ; i < array.length;i++){
    if(array[i].paintings >= 100) {artArray.push(array[i])}
  } return artArray
  /*Your Code Here*/
}
